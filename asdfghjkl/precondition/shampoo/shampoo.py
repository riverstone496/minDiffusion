...
class Preconditioner:
    ...
    def compute_preconditioners(self,num_iters,error_torelance):
        """Compute L^{-1/exp} for each stats matrix L."""
        exp = self.exponent_for_preconditioner()
        eps = self._hps.matrix_eps
        for i, stat in enumerate(self.statistics):
            self.preconditioners[i] = ComputePower(
                stat, exp, ridge_epsilon=eps,iter_count=num_iters,error_tolerance=error_torelance)
    ...
    def step(self, closure=None):
        ...
        for group in self.param_groups:
            ...
            for p in group['params']:
                ...
                if self.hps.interval_scheduler is None:
                ...
                if update_preconditioning_TF:
                    preconditioner.compute_preconditioners(self.hps.num_iters,self.hps.error_torelance)
